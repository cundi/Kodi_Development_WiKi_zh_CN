...
变量可以用在任何支持信息标记的标签中，比如texture，label和visible标签。与应用两个完整的带有条件的控制，它们允许你把条件应用到实际的texture或者label上。
...
某些情况下你想要在皮肤中重复多次使用部分值，于此，你可以定义一个constant：  
...
除了默认控制标签之外还可以使用以下标签。注意

|Tag | Description|
|:--:|:----------:|
|align| Can be left, right, or center. Aligns the text within the given label <width>. Defaults to left   可以是左、右或者中心。排列已有<width>|
|aligny|  Can be top or center. Aligns the text within its given label <height>. Defaults to top|
|scroll|  When true, the text will scroll if longer than the label's <width>. If false, the text will be truncated. Defaults to false. 当该值为true时，如果文本长度大于label的<width>时，文本就会滚动。如果为false，那么文本会被删减。默认值为false。|
